...
def generate_technical_heatmap(tickers_string):
    ...
    for ticker in tickers:
        try:
            ...
            # Calculate slope of the 20-period SMA
            data['SMA20_Slope'] = data['SMA_20'].diff()

            # Get the latest values
            latest = data.iloc[-1]
            indicator_df.loc[ticker, 'RSI'] = latest['RSI_14']
            indicator_df.loc[ticker, 'Above_SMA50'] = 'Yes' if latest['Close'] > latest['SMA_50'] else 'No'
            indicator_df.loc[ticker, 'SMA20_Slope'] = 'Up' if latest['SMA20_Slope'] > 0 else 'Down'

        except Exception:
            continue # Skip tickers that fail

    indicator_df.dropna(how='all', inplace=True)
    if indicator_df.empty: return "Could not generate heatmap for the given tickers."
    ...
    rsi_norm = mcolors.Normalize(vmin=0, vmax=100)
    rsi_colors = plt.cm.RdYlGn(rsi_norm(indicator_df['RSI'].astype(float)))

    table_data = indicator_df.values.T
    cell_colours = [rsi_colors.tolist(), colors, sl_colors]

    table = ax.table(cellText=table_data, rowLabels=indicator_df.columns, colLabels=indicator_df.index,
