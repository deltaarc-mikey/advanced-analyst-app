...
def generate_technical_heatmap(tickers_string):
    ...
    for ticker in tickers:
        try:
            ...
            if data.empty or len(data) < 51: continue
            ...
            latest_close = data['Close'].iloc[-1]; rsi_val = rsi.iloc[-1]; sma50_val = sma50.iloc[-1]
            sma20_val = sma20.iloc[-1]; prev_sma20_val = sma20.iloc[-2]
    ...
    rsi_values = pd.to_numeric(indicator_df['RSI'], errors='coerce'); rsi_norm = mcolors.Normalize(vmin=20, vmax=80)
    rsi_colors = plt.cm.RdYlGn_r(rsi_norm(rsi_values))
    table_data = indicator_df.values.T; cell_colours = [rsi_colors.tolist()] + [colors] + [sl_colors]
...
if 'agent_executor' not in st.session_state:
    ...
    from langchain import hub # Corrected Import Location
