...
def generate_technical_heatmap(tickers_string):
    ...
    for ticker in tickers:
        try:
            ...
            if data.empty or len(data) < 52: continue # Need at least 52 days for reliable calcs
            ...
            # --- More Robustly Get Latest Valid Values ---
            latest_close = data['Close'].dropna().iloc[-1]
            rsi_val = rsi.dropna().iloc[-1]
            sma50_val = sma50.dropna().iloc[-1]
            
            # For slope, we need the last two valid points from the SMA20 series
